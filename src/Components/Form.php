...
class Form
{
    ...
    public static function settingsField($field, $depth = 0)
    {
        ...
        if ($depth > 0) {
            echo '<fieldset style="'.(1 === $depth ? 'display:inline-block;' : '').'box-shadow:inset 0 0 4px #dddddd;padding:10px;margin:.7em 0 .25em;">';
    ...
    public static function toAttributes(&$attributes)
    {
        ...
        foreach ($attributes as $key => $value) {
            if (null !== $value && false !== $value) {
                $result .= ' '.(true === $value ? $key : $key.'="'.htmlspecialchars($value, ENT_QUOTES).'"');
            }
        }
    ...
    public static function text(array $attrs = [])
    {
        ...
        $className = 'number' === $attrs['type'] ? 'small-text' : 'regular-text';
    ...
    {
        if (isset($attrs['multiple']) && false !== $attrs['multiple']) {
            $attrs['name'] .= '[]';
        }
    ...
    public static function checkControlGroup(&$attrs)
    {
        ...
        $options = $attrs['options'];
        $checked = (array) ($attrs['value'] ?? []);
        $space = isset($attrs['direction']) && 'vertical' == $attrs['direction'] ? '</br>' : '&nbsp;&nbsp;';
